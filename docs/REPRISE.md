...
- Toutes les couleurs codées en dur de `src/` (135 teintes, 349 occurrences, hors `src/data`) sont devenues des variables `--c-<hex>` définies dans `src/theme.css` : Le fond de canevas suit le nom du thème (`--canvas-bg`, `--canvas-text`, `--canvas-caption`…), tout le reste prend le contraste inverse : en mode nuit (`:root`, défaut) le canevas est sombre et cartes, menus et panneaux utilisent la table claire (inversion de luminosité de la palette d'origine, accents assombris, neutres réglés à la main) ; en mode clair (`:root[data-theme="light"]`) le canevas est blanc et cartes, menus et panneaux reprennent la palette sombre d'origine à l'identique. Un thème est donc une seule table ; ajouter une couleur en dur réintroduit une valeur non thémée.
